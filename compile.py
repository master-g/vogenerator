...
def main(argv):
    ...
    javaoutput = os.path.join(outputdir, 'build', 'db', 'src', 'main', 'java')
    vooutput = os.path.join(outputdir, 'build')
    ...
    for voRequestFile in argv:
        os.system('protoc -I=' + outputdir +
                  ' --plugin=protoc-gen-vo=protoc-gen-vo ' +
                  '--vo_out=' + vooutput +
                  ' ' + outputdir + os.path.sep + voRequestFile)
